...
class WeatherGroupListTest {
    ...
    fun setup() {
        ...
        repo.apply {
            ...
            every { getWeatherGroup(getCityIdsForException().joinToString(separator = ",")) } returns getWeatherGroupsExceptionResponse()
            every { getWeatherDetails("1701668") } returns getManilaWeather()
            every { getWeatherDetails("3067696") } returns getPragueWeather()
            every { getWeatherDetails("1835848") } returns getSeoulWeather()
        }
    ...
    fun `Fetch weather group API success`(){
        viewModel.run {
            ...
            verify{ groupsObserver.onChanged(capture(weatherGroupsCapture)) }
    ...
    @Test
    fun `Fetch weather group API empty`(){
        viewModel.run {
            fetchWeatherGroups(emptyList())

            val weatherGroupsCapture = slot<List<WeatherItem>>()

            verify{ groupsObserver.onChanged(capture(weatherGroupsCapture)) }

            assertEquals(emptyList<WeatherItem>(), weatherGroupsCapture.captured)
        }

    }

    @Test
    fun `Fetch weather group API with exception`(){
        viewModel.run {
            fetchWeatherGroups(getCityIdsForException())

            val uiCapture = slot<WeatherGroupUi>()

            verify{ uiObserver.onChanged(capture(uiCapture)) }

            assertEquals("Ids not found", uiCapture.captured.error)
        }
